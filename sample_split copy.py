...
if __name__ == '__main__':
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        this_num_samples = len(all_pred_ligand_pos)
        num_samples += this_num_samples

        for sample_idx in range(this_num_samples):
            pos_traj = all_pred_ligand_pos[sample_idx]
            v_traj = all_pred_ligand_v[sample_idx]

            pred_pos = pos_traj[args.eval_step]
            pred_v = v_traj[args.eval_step]
            ...
            try:
                ...
                if args.docking_mode == 'qvina':
                    vina_task = QVinaDockingTask.from_generated_mol(mol, r['data'].ligand_filename, protein_root=args.protein_root)
                    vina_results = vina_task.run_sync()
                elif args.docking_mode in ['vina_score', 'vina_dock']:
                    vina_task = VinaDockingTask.from_generated_mol(mol, r['data'].ligand_filename, protein_root=args.protein_root)
    ...
    if args.save:
        eval_bond_length.plot_distance_hist(success_pair_length_profile, metrics=success_js_metrics,
